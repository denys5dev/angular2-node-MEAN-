...
export class MessageService {
    ...
    addMessage(message: Message) {
        ...
        const token = localStorage.getItem('token')
            ...
            : '';
        return this.http.post(this.serverEntryPoint + 'message' + token, body, {
            headers: headers
        })
    ...
    updateMessage(message: Message) {
        ...
        const token = localStorage.getItem('token')
            ? '?token=' + localStorage.getItem('token')
            : '';
        return this.http.patch(this.serverEntryPoint + 'message/' + message.messageId + token, body, {
            headers: headers
        })
    ...
    deleteMessage(message: Message) {
        ...
        const token = localStorage.getItem('token')
            ? '?token=' + localStorage.getItem('token')
            : '';
        return this.http.delete(this.serverEntryPoint + 'message/' + message.messageId + token)
            .map((response: Response) => response.json())
            .catch((error: Response) => {
                this.errorService.handleError(error.json());
                return Observable.throw(error.json());
